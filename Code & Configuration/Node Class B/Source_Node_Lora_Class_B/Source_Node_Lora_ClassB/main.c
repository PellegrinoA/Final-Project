 ...
 * value in [ms].
 */
#define APP_TX_DUTYCYCLE_RND                        0//5000
 ...
 */
#define LORAWAN_DEFAULT_PING_SLOT_PERIODICITY       1 //2s
...
void PrintHexBuffer( uint8_t *buffer, uint8_t size )
{
    ...
    for( uint8_t i = 0; i < size; i++ )
    {
        ...
        printf( "%c ", buffer[i] ); //Imprimir en HEX %02X
...
int main( void )
{
    ...
    printf( "###### ===== ClassB demo application v1.0.RC1 ==== ######\r\n\r\n" );
     ...
     mibReq.Type = MIB_SYSTEM_MAX_RX_ERROR;
       ...
       mibReq.Param.MinRxSymbols = 40; //8 [Con 50 llega todo. Con 40 tambien. En ABP]
       if( LoRaMacMibSetRequestConfirm( &mibReq ) != LORAMAC_STATUS_OK ){
